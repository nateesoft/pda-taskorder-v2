...
import com.ics.pdatakeorder.model.PKicTranBean;
...
public class HistoryOrder extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int size = listBalance.size();
        List<PKicTranBean> listKicTran = new ArrayList<>();
        if (size > 0) {
            PKicTran kicTran = new PKicTran();
            listKicTran = kicTran.getKicTran(table);
            for (PKicTranBean item : listKicTran) {
                item.setPDesc(ThaiUtil.ASCII2Unicode(item.getPDesc()));
            }
        }
